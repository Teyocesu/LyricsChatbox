...
- Lyric Context: the shared `PresetItem` template gives disabled items the existing raised/line/muted theme treatment after selected, hover and keyboard-focus triggers. Enabled focus visuals remain unchanged, and Home/Display cards remain visible while unsupported controls stay disabled.
...
## Phase 7 — physical QA (additional final candidate smoke waived by product owner)
...
- Final-candidate exception (2026-09-22): the product owner explicitly waived the additional physical smoke of the final ZIP/installer. Record this gate as `WAIVED / NOT PERFORMED BY PRODUCT OWNER`, never as PASS. The previously completed native portable navigation smoke and automated validations remain valid; no installer install/uninstall or real OSC delivery is claimed as validated.
...
- Publication authorization (2026-09-22): the product owner approved publication of this exact candidate and waived the additional final ZIP/installer smoke as recorded in Phase 7. The candidate artifacts are the ZIP and installer listed above, prepared from release-candidate commit `cbabf1a8ef24715e6acf517834e46a6090ace3a8`; the current waiver/authorization update is documentation-only and does not change their payloads. No rebuild is needed. Do not claim installer install/uninstall or real OSC delivery as validated.
